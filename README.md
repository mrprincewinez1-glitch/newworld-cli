...
pytest test_newworld_cli.py

This checks that format_brief() correctly classifies automation potential
(High/Medium/Low) and includes all expected fields in its output.

## Saved briefs

Every brief generated by the program is automatically appended to
briefs.txt in the project folder, so you build a running history of every
automation idea tested — nothing is lost once the terminal closes.
